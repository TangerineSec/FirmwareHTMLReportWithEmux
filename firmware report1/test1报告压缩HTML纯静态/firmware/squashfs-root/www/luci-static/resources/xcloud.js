...
$('.savestatic').live('click',function(){
	...
	if(!checkRepeat(tmpmac)){
		cansave = -2;
		var result = getRepeatDevNameByStr(allstr,repeatData);
		repeatData = "";
		lansetup2mention(result["dev_1"]+"与"+result["dev_2"]+'设备mac地址重复，请重新设置');
		return false;

	}else if( !checkRepeat(tmpip)){
		cansave = -3;
		var result = getRepeatDevNameByStr(allstr,repeatData);
		repeatData = "";
		lansetup2mention(result["dev_1"]+"与"+result["dev_2"]+'设备IP地址重复，请重新设置');
		return false;
	}
	if(cansave != 0){
		return false;
...
function getStrByIndex(index,str){
	var reg="|||";
	var regsub = "||";
	if(index!=-1){
		var tmp1 = 0,tmp2 = 0;
		var len_reg = reg.length;
		tmp1 =  str.indexOf(reg,index) ;
		tmp2 =  str.lastIndexOf(reg,index); 
		var start = tmp1>=tmp2 ? tmp2 : tmp1;
		start = start==-1?0:(start+len_reg);
		var end = tmp1>=tmp2 ? tmp1 : tmp2; 
		var result = str.substr(start,end-len_reg);				
		result = result.split(regsub);		
    	return result;
	}
...
//通过 mac || ip 获取主机名
function getRepeatDevNameByStr(str,findStr){
	var index = str.indexOf(findStr);
	var tmp_1 = getStrByIndex(str.indexOf(findStr),str);
	var tmp_2 = getStrByIndex(str.lastIndexOf(findStr),str);

	return {"dev_1":tmp_1!=-1?tmp_1[0]:-1,"dev_2":tmp_2!=-1?tmp_2[0]:-1};
}

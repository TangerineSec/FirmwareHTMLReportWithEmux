...
function GetResult2_CI(result_xml)
{
	...
	if (GetResult2_CI == "OK")
	{
		...
		if(DemoZWave == 1)
		{
			var Total_Clients_Org = result_xml.Get("GetMultipleHNAPsResponse/GetClientInfoDemoResponse/ClientInfoLists/ClientInfo#");
			var ZWClientIndex = Total_Clients_Org + 1;
			result_xml.Set("GetMultipleHNAPsResponse/GetClientInfoDemoResponse/ClientInfoLists/ClientInfo:"+ZWClientIndex+"/Type", "WiFi_2.4G");
			result_xml.Set("GetMultipleHNAPsResponse/GetClientInfoDemoResponse/ClientInfoLists/ClientInfo:"+ZWClientIndex+"/DeviceName", "ZWave-SmartPlug");
			result_xml.Set("GetMultipleHNAPsResponse/GetClientInfoDemoResponse/ClientInfoLists/ClientInfo:"+ZWClientIndex+"/MacAddress", "00:05:5D:01:55:66");//"D-Link"
			result_xml.Set("GetMultipleHNAPsResponse/GetClientInfoDemoResponse/ClientInfoLists/ClientInfo:"+ZWClientIndex+"/IPv4Address", "");
		}
		//-----ZWave Demo
		...
		if (Client_Type == "Host")
		{
			for (var i = 1; i <= Total_Clients; i ++)
			{
				...
				if (GetClientType == "LAN" || GetClientType == "WiFi_2.4G" || GetClientType == "WiFi_5G")
				{
					...
					/**/	else if (CheckDeviceType.search("ZWave") != -1)	{	array2Dv[count][11] = "ZWave";	}
					...
					if (allText.match(Str_toUpperCase) == Str_toUpperCase)
					{
						...
						array2Dv[count][4] = StrIndexOfNumber_ToString[0];
					}
	...
	if (val != 0)
	{
		...
		for (var i = 1; i <= TCWithMF; i ++)
		{
			...
			if (i % 3 != 0)
			{
				...
				/**/			if (Get_SmartPlugName != "")	{	tmp += "<div class =\"client_Name\">" + Get_SmartPlugName + "</div>";	}
				/**/			else							{	tmp += "<div class =\"client_Name\">" + Get_ClientName + "</div>";		}
				/**/			tmp += "<div class =\"client_EditImage\" onclick=\"event.cancelBubble=true;client_EditMember(" + ClientNumber + ")\" style=\"cursor:pointer;display:none\"></div>";
				/**/			tmp += "<div class =\"client_Vender\">" + Get_ClientVendor + "</div>";
				/**/			tmp += "<div class =\"client_IPv4Address\">" + Get_ClientIPV4Address + "</div>";
				/**/			tmp += "<div class =\"client_IPv6Address\" id=\"IPv6Address_Client_" + ClientNumber + "\">" + Get_ClientIPV6Address + "</div>";
				/**/			if (Get_ClientMacFilter == "Allowed")	{	tmp += "<div class =\"client_Access\">" + I18N("j", "Access") + ": " + "<font color=\"" + Get_FontColor + "\">" +Get_ClientMacFilter + "</font></div>";	}
				...
				/**/			tmp += "</div>";
				/**/			tmp += "<div class =\"back side\">";
				/**/			tmp += "<div class =\"client_PowerButton\" onclick=\"event.cancelBubble=true;ZWave_SmartPlugPowerBtn(this)\"></div>";
				/**/			tmp += "<div class =\"client_SmartPlugInfo\">";
				/**/			tmp += "<div class =\"client_SmartPlugName\">" + Get_ClientName + "</div>";
				/**/			tmp += "<div class =\"watt_Image\"></div>";
				/**/			tmp += "</div>";
...
function closeEditPOP()	{	HNAP.GetXMLAsync("GetClientInfoDemo", null, "GetValue", function(xml)	{	GetResult_GCI_1(xml)	});	}
function GetResult_GCI_1(result_xml)
{
	var GetResult_GCI_1 = result_xml.Get("GetClientInfoDemoResponse/GetClientInfoDemoResult");
	if (GetResult_GCI_1 == "OK")
	{
		if (valueChanged == 1)
		{
			...
			for (var m = 0; m < Total_Device; m ++)
			{
				...
				if (Check_Flag == 1)
				{
					if (Get__DeviceName.length != 0)
					{
						xml_SetClientInfo.Set("SetClientInfoDemo/ClientInfoLists/ClientInfo:" + SetNickName + "/MacAddress", Get__MACAddress);
						if (Get__DeviceName == Get__NickName)	{	xml_SetClientInfo.Set("SetClientInfoDemo/ClientInfoLists/ClientInfo:" + SetNickName + "/NickName", "");					}
						else									{	xml_SetClientInfo.Set("SetClientInfoDemo/ClientInfoLists/ClientInfo:" + SetNickName + "/NickName", Get__DeviceName);	}
						if (document.getElementById("TimmyCheckBox2").checked == true)	{	xml_SetClientInfo.Set("SetClientInfoDemo/ClientInfoLists/ClientInfo:" + SetNickName + "/ReserveIP", Get__ReserveIP);	}
						else															{	xml_SetClientInfo.Set("SetClientInfoDemo/ClientInfoLists/ClientInfo:" + SetNickName + "/ReserveIP", "");				}
					}
					else
					{
						xml_SetClientInfo.Set("SetClientInfoDemo/ClientInfoLists/ClientInfo:" + SetNickName + "/MacAddress", Get__MACAddress);
...
function ZWave_SmartPlugPowerBtn(element)
{
	var xml_SetClientInfo = HNAP.GetXML("SetClientInfoDemo");
	if(element.className == "client_PowerButton")
	{
		element.className = "client_PowerButton_off";
		xml_SetClientInfo.Set("SetClientInfoDemo/ClientInfoLists:1/ClientInfo/SupportedAction/ZWaveSmartPlug", "Off");
	}
	else
	{
		element.className = "client_PowerButton";
		xml_SetClientInfo.Set("SetClientInfoDemo/ClientInfoLists:1/ClientInfo/SupportedAction/ZWaveSmartPlug", "On");
	}
	var xml_SetClientInfoResult = HNAP.SetXML("SetClientInfoDemo", xml_SetClientInfo);
}
...
function openWebUI(val)
{
	...
	switch (checkName)
	{
		case "DNS":
			...
			var deviceIP = "";
			var devicePort = "";
			var NAS_IP = array2Dv[val - 1][2];
			if (CurrentIP.search(".local") != -1 || CurrentIP.search("192.168") != -1)	{	window.open("http://" + NAS_IP, "_blank");	}
			else
			{
				...
				for (var i = 1; i <= GetVS_ListNumber; i ++)
				{
					...
					if (NAS_IP == GetVS_LocalIPAddress)
					{
...
function checkIPCamPW(addr)
{
	...
	tmp += "<applet name=\"cvcs\" codeBase=\"" + deviceIP + "\" archive=\"aplug.jar\" code=\"aplug.class\" width=480 height=360>";

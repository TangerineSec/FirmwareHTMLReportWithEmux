...
function addRowToTable(num)
{
	if (IsLoaded)
	{
		...
		// d = d.replace("/", "");	why?
		if(!d){
			Input2.innerHTML = 'N/A';
		}
		else if (d == DefaultDirectory_RootP || d == DefaultDirectory_Path || d == "root" || d == "/root")	{	Input2.innerHTML = I18N("j", "root");	}
...
function editDataGet(id)
{
	...
	if(tmp_b == 'guest'){
		document.getElementById('edit_Permission').options[1].disabled = true;
	}
	else{
		document.getElementById('edit_Permission').options[1].disabled = false;
	}

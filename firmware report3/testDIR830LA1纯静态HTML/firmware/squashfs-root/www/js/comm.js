...
var xml_encoding_char = new Array('&', '<', '>', ' ');
var xml_encoding_code = new Array('&amp;', '&lt;', '&gt;', '&nbsp;');
...
function decode_char(encode_msg)
{
	var find = false;
	...
	for(i = 0; i < encode_msg.length; i++)
	{
		...
		{
			find = false;
			for(k = 0; k < xml_encoding_code.length; k++)
			{
				tmp_char = xml_encoding_code[k];
				if(encode_msg.substring(i, i + tmp_char.length) == tmp_char)
				{
					msg += xml_encoding_char[k];
					i += tmp_char.length-1;
					find = true;
					break;
				}
			...
			if(find == false)
			{
				tmp_str = encode_msg[i+3] + encode_msg[i+4];	//&#x00
			
				var dec = parseInt(tmp_str, 16);
				
				msg += String.fromCharCode(dec);
				i += 5;
			}	
...
function encode_char(msg)
{
	...
	for(var i = 0; i < msg.length; i++)
	{
		...
		//xml special char
		for(var j = 0; j < xml_encoding_char.length; j++)
		{
			if(ch == xml_encoding_char[j])
			{
				str += xml_encoding_code[j];
				find = true;
				break;
			}
		}
...
// inject Array.prototype.indexOf for old browser
